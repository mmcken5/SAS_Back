...
router.route('/')
    .post(parseUrlencoded, parseJSON, function (request, response) {
        ...
        var newUserShadow = new models.Passwords({
            salt : Salt,
    ...
    .get(parseUrlencoded, parseJSON, function (request, response) {
        ...
        if (!Password) {
            models.Passwords.find(function (error, UserShadow) {
                if (error) response.send(error);
                response.json({password: UserShadow});
            });
        } else {
            if (Password.user) {
                models.Passwords.findOne({"user": Password.user}, function (error, UserShadow) {
                    if (error) response.send(error);
                    response.json({password: UserShadow});
                });
            } else {
                if (Password.userName){
                    models.Passwords.findOne({"userName": Password.userName}, function (error, UserShadow) {
                        if (error) response.send(error);
                        response.json({password: UserShadow});
                    });
                }
...
router.route('/:password_id')
    .get(parseUrlencoded, parseJSON, function (request, response) {
        models.Passwords.findById(request.params.password_id, function (error, UserShadow) {
            if (error) {
                response.send({error: error});
            }
            else {
    ...
    .delete(parseUrlencoded, parseJSON, function (request, response) {
        models.Passwords.findByIdAndRemove(request.params.password_id,
