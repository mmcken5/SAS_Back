...
function getToken(UserShadow, callback ){
    models.UserRoles.find({"user": UserShadow.user}, function (error, userRoles) {
        ...
        if (UserRolesSize === 0) {
            ...
            for (i = 0; i < UserRolesSize; i++) {
                ...
                models.RolePermissions.find({"roleCodes": roleID}, function (error, features) {
                    n++;
                    if (error) response.json({login: failedLogin()});
...
router.route('/')
    .post(parseUrlencoded, parseJSON, function (request, response) {
        models.Passwords.findOne({"userName": request.body.login.userName}, function (error, UserShadow) {
            if (error || !UserShadow) {
                // Username must be wrong, server will send "wrong username" message
                var badUserName = new models.Logins({
                ...
                if (request.body.login.requestType === "open") {// first message in the authentication protocol
                    ...
                    models.Logins.find({"userName": request.body.login.userName}, function (error, oldLogins) {
                        oldLogins.forEach(function (record) {
                            models.Logins.findByIdAndRemove(record.id,
                    ...
                    var newLogin = new models.Logins({
                        userName: request.body.login.userName,
                        password: null,
                    ...
                    if (request.body.login.requestType === "openResponse") { // third message in the authentication protocol
                        ...
                        if (request.body.login.response) {
                            ...
                            models.Logins.findOne({"userName": request.body.login.userName}, function (error, message4) {
                                if (!error) {
                    ...
                    } else
                    {
                        if (request.body.login.requestType === "fetch") { //This is to maintain the instance of the current authenticated session
                            ...
                            var connection = new models.Logins({
                                userName: request.body.login.userName,
                                password: null,
                                nonce: rand(256, 36), // this is the server challenge
                                response: null,
                            ...
                            if (request.body.login.requestType === "fetchResponse") {
                                models.Logins.findOne({"userName": request.body.login.userName}, function (error, Fetch) {
    ...
    .get(parseUrlencoded, parseJSON, function (request, response) {
        ...
        if (!LOGIN) {
            models.Logins.find(function (error, Login) {
                if (error) response.json({login: failedLogin()});
                response.json({login: Login});
            });
        } else {
            models.Logins.findOne({"userName": LOGIN.userName}, function (error, Login) {
                if (error) response.json({login: failedLogin()});
                response.json({login: Login});
            });
        }
...
router.route('/:login_id')
    .delete(parseUrlencoded, parseJSON, function (request, response) {
        models.Logins.findByIdAndRemove(request.params.login_id,
            function (error, deleted) {
                if (!error) {
